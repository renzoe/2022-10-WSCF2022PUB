 ...
 * \ingroup propagation
 * 
 * \brief Returns a received power level based on real measruments previously
 *  set in the model. It is independent of the transmit power
 *
 * The received power is constant independent of the transmit power.  The user
 * must set received power level through the Rss attribute or public
 * SetRss() method.  Note that if this loss model is chained to other loss
 ...
 * that precede it in the chain.
 */
class RealRSSIPropagationLossModel : public PropagationLossModel
{
  public:
    /**
     * \brief Get the type ID.
    ...
    RealRSSIPropagationLossModel();
    ~RealRSSIPropagationLossModel() override;

    // Delete copy constructor and assignment operator to avoid misuse
    RealRSSIPropagationLossModel(const RealRSSIPropagationLossModel&) = delete;
    RealRSSIPropagationLossModel& operator=(const RealRSSIPropagationLossModel&) = delete;

    /**
     * \brief Set RSSI (in dBm) between pair of ns-3 objects (typically, nodes).
     *
     * \param a ma          Source mobility model
     * \param b mb          Destination mobility model
     * \param rssi          a -> b rssi obtained from real measurements
     * \param symmetric     If true (default), both a->b and b->a paths will be affected
     */
    void SetRssi(Ptr<MobilityModel> a, Ptr<MobilityModel> b, double rssi, bool symmetric = true);

    /**
     * Set the default rssi (in dBm) to be used, -150 if not set
     * \param defaultRssi the default RSSI
     */
    void SetDefaultRssi(double defaultRssi);
  ...
  private:
    ...
    double m_default; //!< default RSSI

    /// Typedef: Mobility models pair
    typedef std::pair<Ptr<MobilityModel>, Ptr<MobilityModel>> MobilityPair;

    std::map<MobilityPair, double> m_rssi; //!< RSSI between pair of nodes
};
