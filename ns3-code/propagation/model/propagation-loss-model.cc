...
NS_OBJECT_ENSURE_REGISTERED(RealRSSIPropagationLossModel);

TypeId
RealRSSIPropagationLossModel::GetTypeId()
{
    static TypeId tid =
        TypeId("ns3::RealRSSIPropagationLossModel")
            .SetParent<PropagationLossModel>()
            .SetGroupName("Propagation")
            .AddConstructor<RealRSSIPropagationLossModel>()
            .AddAttribute("DefaultRSSI",
                          "The default value for RSSI, dBm.",
                          DoubleValue(-150.0),
                          MakeDoubleAccessor(&RealRSSIPropagationLossModel::m_default),
                          MakeDoubleChecker<double>());
    return tid;
}

RealRSSIPropagationLossModel::RealRSSIPropagationLossModel()
    : PropagationLossModel()
{
}
...
void
RealRSSIPropagationLossModel::SetDefaultRssi(double rssi)
{
    m_default = rssi;
}

void
RealRSSIPropagationLossModel::SetRssi(Ptr<MobilityModel> ma,
                                    Ptr<MobilityModel> mb,
                                    double rssi,
                                    bool symmetric)
{
    ...
    MobilityPair p = std::make_pair(ma, mb);
    std::map<MobilityPair, double>::iterator i = m_rssi.find(p);

    if (i == m_rssi.end())
    {
        m_rssi.insert(std::make_pair(p, rssi));
    }
    else
    {
        i->second = rssi;
    ...
    if (symmetric)
    {
        SetRssi(mb, ma, rssi, false);
    }
}

double
RealRSSIPropagationLossModel::DoCalcRxPower(double txPowerDbm,
                                          Ptr<MobilityModel> a,
                                          Ptr<MobilityModel> b) const
{
...
int64_t
RealRSSIPropagationLossModel::DoAssignStreams(int64_t stream)
{
    return 0;
}
